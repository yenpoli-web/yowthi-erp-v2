...
| OUT-003 | Outsourced Supply Batch CLOSED then late detail | A | Block normal late detail until real YowThi behavior is confirmed; do not silently reopen within the Business Fact command. Explicit Outsourced Batch Reopen remains DEFERRED after V8-C10 because the current `ConfirmOutsourcedSupplyDetail` command blocks only while the Batch is `CLOSED`; changing it to `ACTIVE` would subsequently allow late detail and therefore decide this unresolved Business Fact. Confirm real late-detail behavior first, or design a control that preserves the unresolved rule safely. |
