...
const copy = {
  'zh-TW': {
    ...
    loading: '載入中…',
    queryFailed: '銷售包裝選項查詢失敗',
    required: '請完整選擇銷售單、員工、項目並填寫工作日期。',
    wageInvalid: '確認工資必須是 0 或正整數泰銖。',
    dailyWageClosed: '此員工該工作日的每日工資已確認。依 LABOR-001，目前不允許一般補登。',
    employeeInactive: '所選員工目前不可用，請重新載入並選擇啟用中的員工。',
    itemInactive: '所選包裝項目目前不可用，請重新載入並選擇啟用中的項目。',
    saleUnavailable: '所選銷售單已不存在或目前狀態不允許此操作。請重新載入。',
    idempotency: '此操作識別碼已被不同請求使用。請重新開始一次登錄。',
    unexpected: '發生未預期錯誤。',
    submit: '登錄包裝工作',
    submitting: '登錄中…',
    success: '包裝工作已登錄',
  ...
  'th-TH': {
    ...
    wage: 'ค่าจ้างที่ยืนยันแล้ว (บาท)',
    wageHint: 'v0.1 บันทึกค่าจ้างที่ยืนยันแล้วโดยตรง และไม่มีช่อง quantity / weight / hour ใน work record นี้',
    loading: 'กำลังโหลด…',
    queryFailed: 'โหลดตัวเลือกงานบรรจุขายไม่สำเร็จ',
    required: 'โปรดเลือกการขาย พนักงาน และรายการ พร้อมระบุวันที่ทำงานให้ครบ',
    wageInvalid: 'ค่าจ้างที่ยืนยันแล้วต้องเป็น 0 หรือจำนวนเต็มบวกบาท',
    dailyWageClosed: 'ค่าจ้างรายวันของพนักงานในวันทำงานนี้ถูกยืนยันแล้ว ตาม LABOR-001 ไม่อนุญาตให้เพิ่มงานย้อนหลังแบบปกติ',
    employeeInactive: 'พนักงานที่เลือกไม่พร้อมใช้งาน โปรดโหลดใหม่และเลือกพนักงานที่ใช้งานอยู่',
    itemInactive: 'รายการบรรจุที่เลือกไม่พร้อมใช้งาน โปรดโหลดใหม่และเลือกรายการที่ใช้งานอยู่',
    saleUnavailable: 'รายการขายที่เลือกไม่มีอยู่หรือสถานะปัจจุบันไม่รองรับการทำงานนี้ โปรดโหลดใหม่',
              ...
              >
                ...
                {(salesQuery.data?.items ?? []).map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.salesDate} · {item.customerDisplayName} · {item.status === 'DRAFT' ? labels.draftStatus : labels.confirmedStatus}
                  </option>
                ))}
              </select>
            </div>
