...
function createPrototypeSupplierId(): string {
  if (typeof globalThis.crypto?.randomUUID === 'function') {
    return `prototype-${globalThis.crypto.randomUUID()}`;
  }
  ...
  return `prototype-${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20)}`;
}

export function createPrototypeSupplier(draft: PrototypeSupplierDraft): PrototypeSupplier {
  const created: PrototypeSupplier = {
    id: createPrototypeSupplierId(),
    ...draft,
    deleted: false,
    deletedAt: null,
    rowVersion: 1,
