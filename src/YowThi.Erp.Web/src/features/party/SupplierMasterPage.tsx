...
const copy = {
  'zh-TW': {
    eyebrow: '夥伴管理', title: '供應商', newSupplier: '新增供應商', search: '搜尋編號、名稱、電話或銀行', supplierTab: '供應商', customerTab: '客戶',
    all: '全部', active: '使用中', inactive: '停用', deleted: '已刪除', supplier: '供應商', listName: '名稱', phone: '電話', status: '狀態', records: '筆資料',
  ...
  'th-TH': {
    eyebrow: 'จัดการคู่ค้า', title: 'ผู้จำหน่าย', newSupplier: 'เพิ่มผู้จำหน่าย', search: 'ค้นหารหัส ชื่อ โทรศัพท์ หรือธนาคาร', supplierTab: 'ผู้จำหน่าย', customerTab: 'ลูกค้า',
    all: 'ทั้งหมด', active: 'ใช้งาน', inactive: 'ไม่ใช้งาน', deleted: 'ลบแล้ว', supplier: 'ผู้จำหน่าย', listName: 'ชื่อ', phone: 'โทรศัพท์', status: 'สถานะ', records: 'รายการ',
  ...
  return (
    <section className="supplier-master-prototype" aria-labelledby="supplier-master-title">
      ...
      <div className="supplier-master-grid">
        <aside className="supplier-list-panel supplier-master-record-list" aria-label={labels.supplier}>
          ...
          <div className="supplier-list-body">
            {items.map((item) => <div key={item.id} role="button" tabIndex={0} className={`supplier-list-item${selected?.id === item.id ? ' is-selected' : ''}`} onClick={() => selectSupplier(item)} onKeyDown={(event) => handleListKey(event, item)}><span className="supplier-list-code">{item.code || labels.notProvided}</span><span className="supplier-list-name"><strong>{displayName(item)}</strong></span></div>)}
