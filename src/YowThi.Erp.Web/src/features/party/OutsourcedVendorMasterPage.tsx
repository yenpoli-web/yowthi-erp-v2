...
import {
  createOutsourcedVendor,
  listOutsourcedVendors,
  updateOutsourcedVendor,
  type OutsourcedVendorDraftRequest,
  type OutsourcedVendorMasterItem,
  type OutsourcedVendorMasterStatus,
} from './outsourcedVendorMaster';
import './supplierMasterPrototype.css';

type EditorMode = 'detail' | 'create' | 'edit';
type OutsourcedVendorDraft = {
  nameZhTw: string;
  nameThTh: string;
  phone: string;
  address: string;
...
const copy = {
  'zh-TW': {
    eyebrow: '夥伴管理', title: '委外供應商', newOutsourcedVendor: '新增委外供應商', search: '搜尋名稱、電話或銀行', supplierTab: '委外供應商', customerTab: '客戶',
    ...
    deleteTitle: '刪除委外供應商', deleteMessage: '這會將資料設為軟刪除；之後可從「已刪除」恢復，並會出現在資料保護的永久刪除清單。', confirmDelete: '確認刪除',
    noResult: '沒有符合條件的委外供應商。', notProvided: '—', queryFailed: '無法載入委外供應商資料', unexpected: '操作失敗，請重新整理後再試。',
    stale: '資料已被其他操作更新，請重新整理後再試。', notFound: '委外供應商資料不存在。', deletedConflict: '這筆委外供應商已經刪除，請先恢復後再編輯。', idempotency: '相同操作識別已被其他內容使用。',
  },
  'th-TH': {
    eyebrow: 'จัดการคู่ค้า', title: 'ผู้รับจ้างภายนอก', newOutsourcedVendor: 'เพิ่มผู้รับจ้างภายนอก', search: 'ค้นหาชื่อ โทรศัพท์ หรือธนาคาร', supplierTab: 'ผู้รับจ้างภายนอก', customerTab: 'ลูกค้า',
    ...
    deleteTitle: 'ลบผู้รับจ้างภายนอก', deleteMessage: 'รายการจะถูกลบแบบเก็บประวัติ สามารถกู้คืนได้ และจะแสดงในพื้นที่คุ้มครองสำหรับการลบถาวร', confirmDelete: 'ยืนยันการลบ',
    noResult: 'ไม่พบผู้รับจ้างภายนอกที่ตรงกับเงื่อนไข', notProvided: '—', queryFailed: 'ไม่สามารถโหลดข้อมูลผู้รับจ้างภายนอกได้', unexpected: 'ดำเนินการไม่สำเร็จ กรุณารีเฟรชแล้วลองใหม่',
    stale: 'ข้อมูลถูกแก้ไขโดยการทำงานอื่น กรุณารีเฟรชแล้วลองใหม่', notFound: 'ไม่พบข้อมูลผู้รับจ้างภายนอก', deletedConflict: 'ผู้รับจ้างภายนอกนี้ถูกลบแล้ว กรุณากู้คืนก่อนแก้ไข', idempotency: 'รหัสการทำงานเดียวกันถูกใช้กับข้อมูลอื่นแล้ว',
  },
} as const;

type Labels = typeof copy['zh-TW'] | typeof copy['th-TH'];

function toDraft(item: OutsourcedVendorMasterItem): OutsourcedVendorDraft {
  return {
...
function emptyDraft(): OutsourcedVendorDraft {
  return { nameZhTw: '', nameThTh: '', phone: '', address: '', bankName: '', bankAccount: '', active: true };
}
function toRequest(draft: OutsourcedVendorDraft): OutsourcedVendorDraftRequest {
...
export function OutsourcedVendorMasterPage() {
  const { locale } = useOperationalLocale();
  const labels = copy[locale];
  const queryClient = useQueryClient();
  const [selectedId, setSelectedId] = useState('');
  const [mode, setMode] = useState<EditorMode>('detail');
  const [draft, setDraft] = useState<OutsourcedVendorDraft>(emptyDraft);
  const [filter, setFilter] = useState<OutsourcedVendorMasterStatus>('all');
  ...
  const outsourcedVendorsQuery = useQuery({
    queryKey: ['outsourced-vendor-master', locale, deferredSearch, filter],
    queryFn: ({ signal }) => listOutsourcedVendors({ locale, search: deferredSearch, status: filter, limit: 200, signal }),
    staleTime: 2_000,
  });
  const items = outsourcedVendorsQuery.data?.items ?? [];
  const selected = items.find((item) => item.id === selectedId)
    ?? (mode === 'detail' ? items[0] ?? null : null);

  async function invalidateOutsourcedVendorData() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['outsourced-vendor-master'] }),
      queryClient.invalidateQueries({ queryKey: ['party-lifecycle-options', 'outsourced-vendors'] }),
      queryClient.invalidateQueries({ queryKey: ['hard-delete-options', 'outsourced-vendors'] }),
    ]);
  }

  const createMutation = useMutation({
    mutationFn: (input: { request: OutsourcedVendorDraftRequest; idempotencyKey: string }) => createOutsourcedVendor(input.request, { locale, idempotencyKey: input.idempotencyKey }),
    onSuccess: async (result) => {
      setFilter('all'); setSelectedId(result.outsourcedVendorId); setMode('detail'); writeIdentity.current = null;
      await invalidateOutsourcedVendorData();
    },
  });
  const updateMutation = useMutation({
    mutationFn: (input: { id: string; expectedRowVersion: number; request: OutsourcedVendorDraftRequest; idempotencyKey: string }) =>
      updateOutsourcedVendor(input.id, { ...input.request, expectedRowVersion: input.expectedRowVersion }, { locale, idempotencyKey: input.idempotencyKey }),
    onSuccess: async (result) => {
      setSelectedId(result.outsourcedVendorId); setMode('detail'); writeIdentity.current = null;
      await invalidateOutsourcedVendorData();
    },
  });
  const lifecycleMutation = useMutation({
    mutationFn: (input: { id: string; action: PartyLifecycleAction; expectedRowVersion: number; idempotencyKey: string }) =>
      changePartyLifecycle('outsourced-vendors', input.id, input.action, { expectedRowVersion: input.expectedRowVersion }, { locale, idempotencyKey: input.idempotencyKey }),
  ...
  const activeError = createMutation.error ?? updateMutation.error ?? lifecycleMutation.error ?? outsourcedVendorsQuery.error;
  const problemMessage = useMemo(() => {
    if (!activeError) return null;
    if (!(activeError instanceof ApiProblemError)) return outsourcedVendorsQuery.error ? labels.queryFailed : labels.unexpected;
    if (activeError.code === 'party.outsourced-vendor.stale-row-version' || activeError.code === 'concurrency.stale-row-version') return labels.stale;
    if (activeError.code === 'party.outsourced-vendor.not-found') return labels.notFound;
    if (activeError.code === 'party.outsourced-vendor.deleted') return labels.deletedConflict;
    if (activeError.code === 'party.outsourced-vendor.idempotency-key-reused' || activeError.code === 'idempotency.key-reused') return labels.idempotency;
    return `${activeError.code}${activeError.problem.traceId ? ` · ${activeError.problem.traceId}` : ''}`;
  }, [activeError, labels, outsourcedVendorsQuery.error]);

  const displayName = (item: OutsourcedVendorMasterItem) => {
  ...
  const secondaryName = (item: OutsourcedVendorMasterItem) => (locale === 'zh-TW' ? item.nameThTh : item.nameZhTw)?.trim() ?? '';

  function selectOutsourcedVendor(item: OutsourcedVendorMasterItem) {
  ...
  function handleListKey(event: KeyboardEvent<HTMLDivElement>, item: OutsourcedVendorMasterItem) {
    if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectOutsourcedVendor(item); }
  }
  ...
  return (
    <section className="supplier-master-prototype" aria-labelledby="outsourced-vendor-master-title">
      <header className="supplier-master-header">
        <div><p className="eyebrow">{labels.eyebrow}</p><h1 id="outsourced-vendor-master-title">{labels.title}</h1></div>
        <div className="supplier-master-header-actions"><PartyMasterNavigation activeKind="outsourced-vendors" /><button className="supplier-primary-action" type="button" onClick={beginCreate} disabled={busy}><span aria-hidden="true">＋</span>{labels.newOutsourcedVendor}</button></div>
      </header>
      ...
      <div className="supplier-master-grid">
        ...
        <article className="supplier-detail-panel">
          {mode === 'create' || mode === 'edit' ? <OutsourcedVendorEditor mode={mode} draft={draft} labels={labels} error={formError ?? problemMessage} busy={busy} onDraftChange={(next) => { setDraft(next); setFormError(null); writeIdentity.current = null; }} onSubmit={saveDraft} onCancel={cancelEditor} /> : selected ? <OutsourcedVendorDetail item={selected} displayName={displayName(selected)} secondaryName={secondaryName(selected)} labels={labels} busy={busy} onEdit={beginEdit} onToggleActive={() => submitUpdateActive(!selected.active)} onDelete={() => setDeleteOpen(true)} onRestore={() => submitLifecycle('restore')} /> : <div className="supplier-list-empty">{outsourcedVendorsQuery.isPending ? '…' : problemMessage ?? labels.noResult}</div>}
        </article>
      </div>

      {deleteOpen && selected && <div className="supplier-dialog-backdrop" role="presentation" onMouseDown={() => setDeleteOpen(false)}><div className="supplier-dialog" role="dialog" aria-modal="true" aria-labelledby="outsourcedVendor-delete-dialog-title" onMouseDown={(event) => event.stopPropagation()}><div className="supplier-dialog-icon" aria-hidden="true">!</div><div><h2 id="outsourcedVendor-delete-dialog-title">{labels.deleteTitle}</h2><p>{displayName(selected)}</p><p className="supplier-dialog-message">{labels.deleteMessage}</p></div><div className="supplier-dialog-actions"><button type="button" className="supplier-secondary-action" onClick={() => setDeleteOpen(false)}>{labels.cancel}</button><button type="button" className="supplier-danger-action" disabled={busy} onClick={() => submitLifecycle('soft-delete')}>{labels.confirmDelete}</button></div></div></div>}
    </section>
  );
}

function OutsourcedVendorDetail({ item, displayName, secondaryName, labels, busy, onEdit, onToggleActive, onDelete, onRestore }: { item: OutsourcedVendorMasterItem; displayName: string; secondaryName: string; labels: Labels; busy: boolean; onEdit: () => void; onToggleActive: () => void; onDelete: () => void; onRestore: () => void }) {
...
function StatusPill({ item, labels }: { item: OutsourcedVendorMasterItem; labels: Labels }) { const state = item.deletedAt ? 'deleted' : item.active ? 'active' : 'inactive'; return <span className={`supplier-status-pill is-${state}`}>{labels[state]}</span>; }
