...
import {
  ...
  listFarmers,
  updateFarmer,
  type FarmerDraftRequest,
  type FarmerMasterItem,
  type FarmerMasterStatus,
} from './farmerMaster';
...
const copy = {
  'zh-TW': {
    eyebrow: '夥伴管理', title: '農戶', newFarmer: '新增農戶', search: '搜尋編號、名稱、電話或銀行', supplierTab: '農戶', customerTab: '客戶',
    all: '全部', active: '使用中', inactive: '停用', deleted: '已刪除', farmer: '農戶', phone: '電話', status: '狀態', records: '筆資料',
    ...
    activeState: '啟用狀態', nameRequired: '中文名稱與泰文名稱至少填寫一項。', save: '儲存', saving: '儲存中…', cancel: '取消', createTitle: '新增農戶', editTitle: '編輯農戶',
    deleteTitle: '刪除農戶', deleteMessage: '這會將資料設為軟刪除；之後可從「已刪除」恢復，並會出現在資料保護的永久刪除清單。', confirmDelete: '確認刪除',
    noResult: '沒有符合條件的農戶。', notProvided: '—', queryFailed: '無法載入農戶資料', unexpected: '操作失敗，請重新整理後再試。',
    stale: '資料已被其他操作更新，請重新整理後再試。', notFound: '農戶資料不存在。', deletedConflict: '這筆農戶已經刪除，請先恢復後再編輯。', idempotency: '相同操作識別已被其他內容使用。',
  },
  'th-TH': {
    eyebrow: 'จัดการคู่ค้า', title: 'เกษตรกร', newFarmer: 'เพิ่มเกษตรกร', search: 'ค้นหารหัส ชื่อ โทรศัพท์ หรือธนาคาร', supplierTab: 'เกษตรกร', customerTab: 'ลูกค้า',
    all: 'ทั้งหมด', active: 'ใช้งาน', inactive: 'ไม่ใช้งาน', deleted: 'ลบแล้ว', farmer: 'เกษตรกร', phone: 'โทรศัพท์', status: 'สถานะ', records: 'รายการ',
    ...
    activeState: 'สถานะการใช้งาน', nameRequired: 'ต้องระบุชื่อภาษาจีนหรือชื่อภาษาไทยอย่างน้อยหนึ่งรายการ', save: 'บันทึก', saving: 'กำลังบันทึก…', cancel: 'ยกเลิก', createTitle: 'เพิ่มเกษตรกร', editTitle: 'แก้ไขเกษตรกร',
    deleteTitle: 'ลบเกษตรกร', deleteMessage: 'รายการจะถูกลบแบบเก็บประวัติ สามารถกู้คืนได้ และจะแสดงในพื้นที่คุ้มครองสำหรับการลบถาวร', confirmDelete: 'ยืนยันการลบ',
    noResult: 'ไม่พบเกษตรกรที่ตรงกับเงื่อนไข', notProvided: '—', queryFailed: 'ไม่สามารถโหลดข้อมูลเกษตรกรได้', unexpected: 'ดำเนินการไม่สำเร็จ กรุณารีเฟรชแล้วลองใหม่',
    stale: 'ข้อมูลถูกแก้ไขโดยการทำงานอื่น กรุณารีเฟรชแล้วลองใหม่', notFound: 'ไม่พบข้อมูลเกษตรกร', deletedConflict: 'เกษตรกรนี้ถูกลบแล้ว กรุณากู้คืนก่อนแก้ไข', idempotency: 'รหัสการทำงานเดียวกันถูกใช้กับข้อมูลอื่นแล้ว',
  },
} as const;

type Labels = typeof copy['zh-TW'] | typeof copy['th-TH'];

function toDraft(item: FarmerMasterItem): FarmerDraft {
  return {
...
function toRequest(draft: FarmerDraft): FarmerDraftRequest {
...
export function FarmerMasterPage() {
  ...
  const [filter, setFilter] = useState<FarmerMasterStatus>('all');
  ...
  const farmersQuery = useQuery({
    queryKey: ['farmer-master', locale, deferredSearch, filter],
    queryFn: ({ signal }) => listFarmers({ locale, search: deferredSearch, status: filter, limit: 200, signal }),
    staleTime: 2_000,
  });
  ...
  async function invalidateFarmerData() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['farmer-master'] }),
      queryClient.invalidateQueries({ queryKey: ['party-lifecycle-options', 'farmers'] }),
      queryClient.invalidateQueries({ queryKey: ['hard-delete-options', 'farmers'] }),
    ]);
  }

  const createMutation = useMutation({
    mutationFn: (input: { request: FarmerDraftRequest; idempotencyKey: string }) => createFarmer(input.request, { locale, idempotencyKey: input.idempotencyKey }),
    onSuccess: async (result) => {
      setFilter('all'); setSelectedId(result.farmerId); setMode('detail'); writeIdentity.current = null;
      await invalidateFarmerData();
    },
  });
  const updateMutation = useMutation({
    mutationFn: (input: { id: string; expectedRowVersion: number; request: FarmerDraftRequest; idempotencyKey: string }) =>
      updateFarmer(input.id, { ...input.request, expectedRowVersion: input.expectedRowVersion }, { locale, idempotencyKey: input.idempotencyKey }),
    onSuccess: async (result) => {
      setSelectedId(result.farmerId); setMode('detail'); writeIdentity.current = null;
      await invalidateFarmerData();
    },
  });
  const lifecycleMutation = useMutation({
    mutationFn: (input: { id: string; action: PartyLifecycleAction; expectedRowVersion: number; idempotencyKey: string }) =>
      changePartyLifecycle('farmers', input.id, input.action, { expectedRowVersion: input.expectedRowVersion }, { locale, idempotencyKey: input.idempotencyKey }),
  ...
  const activeError = createMutation.error ?? updateMutation.error ?? lifecycleMutation.error ?? farmersQuery.error;
  const problemMessage = useMemo(() => {
    if (!activeError) return null;
    if (!(activeError instanceof ApiProblemError)) return farmersQuery.error ? labels.queryFailed : labels.unexpected;
    if (activeError.code === 'party.farmer.stale-row-version' || activeError.code === 'concurrency.stale-row-version') return labels.stale;
    if (activeError.code === 'party.farmer.not-found') return labels.notFound;
    if (activeError.code === 'party.farmer.deleted') return labels.deletedConflict;
    if (activeError.code === 'party.farmer.idempotency-key-reused' || activeError.code === 'idempotency.key-reused') return labels.idempotency;
    return `${activeError.code}${activeError.problem.traceId ? ` · ${activeError.problem.traceId}` : ''}`;
  }, [activeError, labels, farmersQuery.error]);

  const displayName = (item: FarmerMasterItem) => {
  ...
  const secondaryName = (item: FarmerMasterItem) => (locale === 'zh-TW' ? item.nameThTh : item.nameZhTw)?.trim() ?? '';

  function selectFarmer(item: FarmerMasterItem) {
  ...
  function handleListKey(event: KeyboardEvent<HTMLDivElement>, item: FarmerMasterItem) {
    if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); selectFarmer(item); }
  }
  ...
  return (
    <section className="supplier-master-prototype" aria-labelledby="farmer-master-title">
      <header className="supplier-master-header">
        <div><p className="eyebrow">{labels.eyebrow}</p><h1 id="farmer-master-title">{labels.title}</h1></div>
        <div className="supplier-master-header-actions"><PartyMasterNavigation activeKind="farmers" /><button className="supplier-primary-action" type="button" onClick={beginCreate} disabled={busy}><span aria-hidden="true">＋</span>{labels.newFarmer}</button></div>
      </header>
      ...
      <div className="supplier-master-grid">
        <aside className="supplier-list-panel" aria-label={labels.farmer}>
          <div className="supplier-list-meta"><strong>{labels.farmer}</strong><span>{items.length} {labels.records}</span></div>
          <div className="supplier-list-heading" aria-hidden="true"><span>{labels.farmer}</span><span>{labels.phone}</span><span>{labels.status}</span></div>
          <div className="supplier-list-body">
            {items.map((item) => <div key={item.id} role="button" tabIndex={0} className={`supplier-list-item${selected?.id === item.id ? ' is-selected' : ''}`} onClick={() => selectFarmer(item)} onKeyDown={(event) => handleListKey(event, item)}><span className="supplier-list-name"><strong>{displayName(item)}</strong>{secondaryName(item) && <small>{secondaryName(item)}</small>}</span><span className="supplier-list-phone">{item.phone || labels.notProvided}</span><StatusPill item={item} labels={labels} /></div>)}
            {!farmersQuery.isPending && items.length === 0 && <p className="supplier-list-empty">{labels.noResult}</p>}
          </div>
        </aside>

        <article className="supplier-detail-panel">
          {mode === 'create' || mode === 'edit' ? <FarmerEditor mode={mode} draft={draft} labels={labels} error={formError ?? problemMessage} busy={busy} onDraftChange={(next) => { setDraft(next); setFormError(null); writeIdentity.current = null; }} onSubmit={saveDraft} onCancel={cancelEditor} /> : selected ? <FarmerDetail item={selected} displayName={displayName(selected)} secondaryName={secondaryName(selected)} labels={labels} busy={busy} onEdit={beginEdit} onToggleActive={() => submitUpdateActive(!selected.active)} onDelete={() => setDeleteOpen(true)} onRestore={() => submitLifecycle('restore')} /> : <div className="supplier-list-empty">{farmersQuery.isPending ? '…' : problemMessage ?? labels.noResult}</div>}
        </article>
      </div>

      {deleteOpen && selected && <div className="supplier-dialog-backdrop" role="presentation" onMouseDown={() => setDeleteOpen(false)}><div className="supplier-dialog" role="dialog" aria-modal="true" aria-labelledby="farmer-delete-dialog-title" onMouseDown={(event) => event.stopPropagation()}><div className="supplier-dialog-icon" aria-hidden="true">!</div><div><h2 id="farmer-delete-dialog-title">{labels.deleteTitle}</h2><p>{displayName(selected)}</p><p className="supplier-dialog-message">{labels.deleteMessage}</p></div><div className="supplier-dialog-actions"><button type="button" className="supplier-secondary-action" onClick={() => setDeleteOpen(false)}>{labels.cancel}</button><button type="button" className="supplier-danger-action" disabled={busy} onClick={() => submitLifecycle('soft-delete')}>{labels.confirmDelete}</button></div></div></div>}
    </section>
  );
}

function FarmerDetail({ item, displayName, secondaryName, labels, busy, onEdit, onToggleActive, onDelete, onRestore }: { item: FarmerMasterItem; displayName: string; secondaryName: string; labels: Labels; busy: boolean; onEdit: () => void; onToggleActive: () => void; onDelete: () => void; onRestore: () => void }) {
...
function FarmerEditor({ mode, draft, labels, error, busy, onDraftChange, onSubmit, onCancel }: { mode: 'create' | 'edit'; draft: FarmerDraft; labels: Labels; error: string | null; busy: boolean; onDraftChange: (draft: FarmerDraft) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void }) {
  const update = <K extends keyof FarmerDraft>(key: K, value: FarmerDraft[K]) => onDraftChange({ ...draft, [key]: value });
  return <form className="supplier-editor" onSubmit={onSubmit}><header className="supplier-detail-header supplier-editor-header"><h2>{mode === 'create' ? labels.createTitle : labels.editTitle}</h2></header><FormSection title={labels.basicInfo}><Field label={labels.code}><input value={draft.code} onChange={(event) => update('code', event.target.value)} /></Field><Field label={labels.nameZhTw}><input value={draft.nameZhTw} onChange={(event) => update('nameZhTw', event.target.value)} /></Field><Field label={labels.nameThTh}><input value={draft.nameThTh} onChange={(event) => update('nameThTh', event.target.value)} /></Field><fieldset className="supplier-state-fieldset supplier-form-wide"><legend className="field-label">{labels.activeState}</legend><label className={draft.active ? 'is-selected' : ''}><input type="radio" name="farmer-active-state" checked={draft.active} onChange={() => update('active', true)} />{labels.active}</label><label className={!draft.active ? 'is-selected' : ''}><input type="radio" name="farmer-active-state" checked={!draft.active} onChange={() => update('active', false)} />{labels.inactive}</label></fieldset></FormSection><FormSection title={labels.contactInfo}><Field label={labels.phone}><input value={draft.phone} inputMode="tel" onChange={(event) => update('phone', event.target.value)} /></Field><Field label={labels.address} wide><textarea rows={3} value={draft.address} onChange={(event) => update('address', event.target.value)} /></Field></FormSection><FormSection title={labels.paymentInfo}><Field label={labels.bankName}><input value={draft.bankName} onChange={(event) => update('bankName', event.target.value)} /></Field><Field label={labels.bankAccount}><input value={draft.bankAccount} onChange={(event) => update('bankAccount', event.target.value)} /></Field></FormSection>{error && <div className="problem-banner" role="alert">{error}</div>}<footer className="supplier-editor-footer"><button className="supplier-secondary-action" type="button" disabled={busy} onClick={onCancel}>{labels.cancel}</button><button className="supplier-primary-action" type="submit" disabled={busy}>{busy ? labels.saving : labels.save}</button></footer></form>;
}
...
function StatusPill({ item, labels }: { item: FarmerMasterItem; labels: Labels }) { const state = item.deletedAt ? 'deleted' : item.active ? 'active' : 'inactive'; return <span className={`supplier-status-pill is-${state}`}>{labels[state]}</span>; }
