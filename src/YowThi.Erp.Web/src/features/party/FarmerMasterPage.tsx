...
const copy = {
  'zh-TW': {
    ...
    all: '全部', active: '使用中', inactive: '停用', deleted: '已刪除', farmer: '農戶', listName: '姓名', phone: '電話', status: '狀態', records: '筆資料',
  ...
  'th-TH': {
    ...
    all: 'ทั้งหมด', active: 'ใช้งาน', inactive: 'ไม่ใช้งาน', deleted: 'ลบแล้ว', farmer: 'เกษตรกร', listName: 'ชื่อ', phone: 'โทรศัพท์', status: 'สถานะ', records: 'รายการ',
  ...
  return (
    <section className="supplier-master-prototype" aria-labelledby="farmer-master-title">
      ...
      <div className="supplier-master-grid">
        <aside className="supplier-list-panel supplier-master-record-list" aria-label={labels.farmer}>
          ...
          <div className="supplier-list-body">
            {items.map((item) => <div key={item.id} role="button" tabIndex={0} className={`supplier-list-item${selected?.id === item.id ? ' is-selected' : ''}`} onClick={() => selectFarmer(item)} onKeyDown={(event) => handleListKey(event, item)}><span className="supplier-list-code">{item.code || labels.notProvided}</span><span className="supplier-list-name"><strong>{displayName(item)}</strong></span></div>)}
