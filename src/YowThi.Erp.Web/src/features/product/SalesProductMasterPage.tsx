...
const copy = {
  'zh-TW': {
    ...
    edit: '編輯', activate: '啟用', deactivate: '停用', softDelete: '刪除', restore: '恢復', save: '儲存', saving: '儲存中…', cancel: '取消', createTitle: '新增銷售產品', editTitle: '編輯銷售產品',
    deleteTitle: '刪除銷售產品', deleteMessage: '這會將資料設為軟刪除；之後可從「已刪除」恢復。', confirmDelete: '確認刪除',
    noResult: '沒有符合條件的銷售產品。', required: '請確認產品群組、名稱與計價欄位。', queryFailed: '無法載入銷售產品', unexpected: '操作失敗，請重新整理後再試。',
    stale: '資料已被其他操作更新，請重新整理後再試。', notFound: '銷售產品或參考資料不存在。', deletedConflict: '這筆產品已刪除，不能修改。', idempotency: '相同操作識別已被其他內容使用。', reauth: '刪除前必須重新驗證登入身分。', notProvided: '—',
  },
  'th-TH': {
    ...
    edit: 'แก้ไข', activate: 'เปิดใช้งาน', deactivate: 'ปิดใช้งาน', softDelete: 'ลบ', restore: 'กู้คืน', save: 'บันทึก', saving: 'กำลังบันทึก…', cancel: 'ยกเลิก', createTitle: 'เพิ่มสินค้าขาย', editTitle: 'แก้ไขสินค้าขาย',
    deleteTitle: 'ลบสินค้าขาย', deleteMessage: 'รายการจะถูกลบแบบเก็บประวัติ และสามารถกู้คืนได้ภายหลัง', confirmDelete: 'ยืนยันการลบ',
    ...
    stale: 'ข้อมูลถูกแก้ไขแล้ว กรุณารีเฟรชแล้วลองใหม่', notFound: 'ไม่พบสินค้าขายหรือข้อมูลอ้างอิง', deletedConflict: 'สินค้านี้ถูกลบแล้วและไม่สามารถแก้ไขได้', idempotency: 'รหัสการทำงานเดียวกันถูกใช้กับข้อมูลอื่นแล้ว', reauth: 'ต้องยืนยันตัวตนในการเข้าสู่ระบบอีกครั้งก่อนลบ', notProvided: '—',
  },
} as const;
...
export function SalesProductMasterPage() {
  ...
  const [filter, setFilter] = useState<MasterStatus>('all'); const [search, setSearch] = useState(''); const deferredSearch = useDeferredValue(search); const [formError, setFormError] = useState<string | null>(null); const [deleteOpen, setDeleteOpen] = useState(false); const identity = useRef<SubmissionIdentity | null>(null); const lifecycleIdentity = useRef<SubmissionIdentity | null>(null);
  ...
  const lifecycleMutation = useMutation({ mutationFn: (input: { id: string; action: ProductMasterLifecycleAction; rowVersion: number; idempotencyKey: string }) => changeSalesProductLifecycle(input.id, input.action, input.rowVersion, { locale, idempotencyKey: input.idempotencyKey }), onSuccess: async (_, input) => { setFilter(input.action === 'soft-delete' ? 'deleted' : 'all'); setDeleteOpen(false); lifecycleIdentity.current = null; await invalidate(); } });
  async function invalidate() { await Promise.all([queryClient.invalidateQueries({ queryKey: ['sales-product-master'] }), queryClient.invalidateQueries({ queryKey: ['sales-confirmation-options'] }), queryClient.invalidateQueries({ queryKey: ['outsourced-supply-detail-options'] }), queryClient.invalidateQueries({ queryKey: ['hard-delete-options', 'sales-products'] })]); }

  const activeError = createMutation.error ?? updateMutation.error ?? lifecycleMutation.error ?? itemsQuery.error ?? groupsQuery.error ?? storageQuery.error;
  ...
  function select(item: SalesProductItem) { setSelectedId(item.id); setDraft(toDraft(item)); setMode('detail'); setFormError(null); setDeleteOpen(false); identity.current = null; lifecycleIdentity.current = null; }
  function beginCreate() { setSelectedId(''); setDraft(emptyDraft()); setMode('create'); setFormError(null); setDeleteOpen(false); identity.current = null; lifecycleIdentity.current = null; }
