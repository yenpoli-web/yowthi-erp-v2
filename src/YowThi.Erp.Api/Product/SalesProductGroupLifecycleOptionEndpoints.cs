...
public static class SalesProductGroupLifecycleOptionEndpoints
{
    public const string OperationId = "Product_ListSalesProductGroupLifecycleOptions";

    private const int DefaultLimit = 50;
    private const int MaxLimit = 100;

    public static IEndpointRouteBuilder MapSalesProductGroupLifecycleOptionEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ArgumentNullException.ThrowIfNull(endpoints);

        endpoints.MapApiV1()
            .MapGet("/product/lifecycle-options/sales-product-groups", ListAsync)
            .WithName(OperationId)
            .RequireAuthorization(CapabilityPolicies.SalesProductGroupLifecycle)
            .Produces<SalesProductGroupLifecycleOptionsResponse>(StatusCodes.Status200OK)
    ...
    private static async Task<IResult> ListAsync(
        ...
        [FromServices] ISalesProductGroupLifecycleOptionsReader reader,
        CancellationToken cancellationToken)
    {
        ...
        var page = await reader.GetAsync(query!, cancellationToken);
        return TypedResults.Ok(new SalesProductGroupLifecycleOptionsResponse(
            page.Items.Select(item => new SalesProductGroupLifecycleOptionResponse(
                item.Id,
                item.DisplayName,
                item.Active,
                item.RowVersion,
    ...
    private static bool TryCreateQuery(
        ...
        out SalesProductGroupLifecycleOptionsQuery? query,
        out IResult? error)
    {
        ...
        query = new SalesProductGroupLifecycleOptionsQuery(
...
public sealed record SalesProductGroupLifecycleOptionResponse(
    Guid Id,
    string DisplayName,
    bool Active,
    long RowVersion,
...
public sealed record SalesProductGroupLifecycleOptionsResponse(
    IReadOnlyList<SalesProductGroupLifecycleOptionResponse> Items,
    string? NextCursor);
