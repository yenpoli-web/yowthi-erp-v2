...
internal sealed class PostgreSqlSalesProductGroupLifecycleExecutor : ISalesProductGroupLifecycleExecutor
{
    private const string SoftDeleteCommandType = "SoftDeleteSalesProductGroup";
    private const string RestoreCommandType = "RestoreSalesProductGroup";
    ...
    public PostgreSqlSalesProductGroupLifecycleExecutor(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validationError = SalesProductGroupLifecycleValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<SalesProductGroupLifecycleResult>.Failure(validationError));
        }
    ...
    private async ValueTask<ApplicationResult<SalesProductGroupLifecycleResult>> ExecuteAsync(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<SalesProductGroupLifecycleResult>>.Rollback(
                        ApplicationResult<SalesProductGroupLifecycleResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        SalesProductGroupLifecycleErrorCodes.IdempotencyKeyReused);
                }

                var state = await LockGroupAsync(salesProductGroupId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        SalesProductGroupLifecycleErrorCodes.GroupNotFound);
                }
                ...
                if (state.RowVersion != expectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesProductGroupLifecycleErrorCodes.StaleRowVersion);
                }
                ...
                if (operation == LifecycleOperation.SoftDelete && state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesProductGroupLifecycleErrorCodes.AlreadyDeleted);
                }
                ...
                if (operation == LifecycleOperation.Restore && state.DeletedAt is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesProductGroupLifecycleErrorCodes.NotDeleted);
                }
                ...
                var result = new SalesProductGroupLifecycleResult(
                    salesProductGroupId,
                    newRowVersion.Value,
                ...
                return CommandTransactionDecision<ApplicationResult<SalesProductGroupLifecycleResult>>.Commit(
                    ApplicationResult<SalesProductGroupLifecycleResult>.Success(result));
            },
            cancellationToken);
    }

    private async ValueTask<GroupState?> LockGroupAsync(
        Guid salesProductGroupId,
        CancellationToken cancellationToken)
    {
        await using var command = CreateSqlCommand(
            ...
            FOR UPDATE;
            """);
        command.Parameters.AddWithValue("group_id", salesProductGroupId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Sales Product Group lifecycle CommandExecution conflict could not be loaded.");
        }
        ...
        var replay = JsonSerializer.Deserialize<SalesProductGroupLifecycleResult>(reader.GetString(4), StoredJsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Sales Product Group lifecycle CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Sales Product Group lifecycle SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<SalesProductGroupLifecycleResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<SalesProductGroupLifecycleResult>>.Rollback(
            ApplicationResult<SalesProductGroupLifecycleResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record GroupState(long RowVersion, DateTimeOffset? DeletedAt);
    ...
    private sealed record CommandAcquisition(
        ...
        SalesProductGroupLifecycleResult? ReplayResult)
    {
        public static CommandAcquisition Acquired() => new(CommandAcquisitionKind.Acquired, null);
        public static CommandAcquisition Replay(SalesProductGroupLifecycleResult result) => new(CommandAcquisitionKind.Replay, result);
