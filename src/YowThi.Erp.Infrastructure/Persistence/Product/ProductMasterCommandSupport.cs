...
internal sealed class ProductMasterCommandSupport
{
    private static readonly JsonSerializerOptions StoredJsonOptions = new(JsonSerializerDefaults.Web);
    private readonly ErpDbContext _dbContext;

    public ProductMasterCommandSupport(ErpDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async ValueTask<CommandAcquisition<TResult>> AcquireAsync<TResult>(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Product master CommandExecution conflict could not be loaded.");
        }
    ...
    public async ValueTask MarkSucceededAsync(CommandId commandId, object result, DateTimeOffset executedAt, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Product master CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    public NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Product master SQL requires an active command transaction.");
...
internal enum CommandAcquisitionKind
{
    Acquired,
    Replay,
    Conflict,
...
internal sealed record CommandAcquisition<TResult>(CommandAcquisitionKind Kind, TResult? ReplayResult)
{
