...
internal sealed class PostgreSqlCustomerLifecycleExecutor : ICustomerLifecycleExecutor
{
    private const string SoftDeleteCommandType = "SoftDeleteCustomer";
    private const string RestoreCommandType = "RestoreCustomer";
    ...
    public PostgreSqlCustomerLifecycleExecutor(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validationError = CustomerLifecycleValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<CustomerLifecycleResult>.Failure(validationError));
        }
    ...
    private async ValueTask<ApplicationResult<CustomerLifecycleResult>> ExecuteAsync(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<CustomerLifecycleResult>>.Rollback(
                        ApplicationResult<CustomerLifecycleResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        CustomerLifecycleErrorCodes.IdempotencyKeyReused);
                }

                var state = await LockCustomerAsync(customerId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        CustomerLifecycleErrorCodes.CustomerNotFound);
                }
                ...
                if (state.RowVersion != expectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        CustomerLifecycleErrorCodes.StaleRowVersion);
                }
                ...
                if (operation == LifecycleOperation.SoftDelete && state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        CustomerLifecycleErrorCodes.AlreadyDeleted);
                }
                ...
                if (operation == LifecycleOperation.Restore && state.DeletedAt is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        CustomerLifecycleErrorCodes.NotDeleted);
                }
                ...
                var result = new CustomerLifecycleResult(
                    customerId,
                    newRowVersion.Value,
                ...
                return CommandTransactionDecision<ApplicationResult<CustomerLifecycleResult>>.Commit(
                    ApplicationResult<CustomerLifecycleResult>.Success(result));
            },
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Customer lifecycle CommandExecution conflict could not be loaded.");
        }
        ...
        var replay = JsonSerializer.Deserialize<CustomerLifecycleResult>(
            reader.GetString(4),
            StoredJsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Customer lifecycle CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Customer lifecycle SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<CustomerLifecycleResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<CustomerLifecycleResult>>.Rollback(
            ApplicationResult<CustomerLifecycleResult>.Failure(ApplicationError.Create(kind, code)));
    ...
    private sealed record CommandAcquisition(
        ...
        CustomerLifecycleResult? ReplayResult)
    {
        public static CommandAcquisition Acquired() => new(CommandAcquisitionKind.Acquired, null);
        public static CommandAcquisition Replay(CustomerLifecycleResult result) => new(CommandAcquisitionKind.Replay, result);
