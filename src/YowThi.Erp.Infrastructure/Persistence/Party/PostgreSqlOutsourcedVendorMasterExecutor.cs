...
internal sealed class PostgreSqlOutsourcedVendorMasterExecutor : IOutsourcedVendorMasterExecutor
{
    private const string CreateCommandType = "CreateOutsourcedVendor";
    private const string UpdateCommandType = "UpdateOutsourcedVendor";
    ...
    public PostgreSqlOutsourcedVendorMasterExecutor(
    ...
    public ValueTask<ApplicationResult<OutsourcedVendorMasterWriteResult>> CreateAsync(
        CreateOutsourcedVendorExecution execution,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(execution);
        var validationError = OutsourcedVendorMasterValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<OutsourcedVendorMasterWriteResult>.Failure(validationError));
        }
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var acquisition = await AcquireCommandAsync<OutsourcedVendorMasterWriteResult>(
                    execution.CommandId,
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<OutsourcedVendorMasterWriteResult>>.Rollback(
                        ApplicationResult<OutsourcedVendorMasterWriteResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        OutsourcedVendorMasterErrorCodes.IdempotencyKeyReused);
                }

                var outsourcedVendorId = Guid.CreateVersion7();
                await InsertOutsourcedVendorAsync(
                    outsourcedVendorId,
                    execution.Command,
                ...
                var result = new OutsourcedVendorMasterWriteResult(outsourcedVendorId, 1);
                ...
                return CommandTransactionDecision<ApplicationResult<OutsourcedVendorMasterWriteResult>>.Commit(
                    ApplicationResult<OutsourcedVendorMasterWriteResult>.Success(result));
            },
            cancellationToken);
    }

    public ValueTask<ApplicationResult<OutsourcedVendorMasterWriteResult>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var state = await LockOutsourcedVendorAsync(execution.Command.OutsourcedVendorId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        OutsourcedVendorMasterErrorCodes.OutsourcedVendorNotFound);
                }
                ...
                if (state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        OutsourcedVendorMasterErrorCodes.OutsourcedVendorDeleted);
                }
                ...
                if (state.RowVersion != execution.Command.ExpectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        OutsourcedVendorMasterErrorCodes.StaleRowVersion);
                }

                var nextRowVersion = await UpdateOutsourcedVendorAsync(execution.Command, ct);
                ...
                var result = new OutsourcedVendorMasterWriteResult(
    ...
    private async ValueTask InsertOutsourcedVendorAsync(
        ...
        CancellationToken cancellationToken)
    {
        await using var insert = CreateSqlCommand(
            ...
            """);
        insert.Parameters.AddWithValue("id", outsourcedVendorId);
    ...
    private async ValueTask<OutsourcedVendorState?> LockOutsourcedVendorAsync(
        Guid outsourcedVendorId,
        CancellationToken cancellationToken)
    {
        ...
        return new OutsourcedVendorState(
            reader.GetInt64(0),
            reader.IsDBNull(1) ? null : reader.GetFieldValue<DateTimeOffset>(1));
    }

    private async ValueTask<long?> UpdateOutsourcedVendorAsync(
        ...
        CancellationToken cancellationToken)
    {
        await using var update = CreateSqlCommand(
            ...
            """);
        update.Parameters.AddWithValue("outsourced_vendor_id", command.OutsourcedVendorId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("OutsourcedVendor master CommandExecution conflict could not be loaded.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("OutsourcedVendor master CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("OutsourcedVendor master SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<OutsourcedVendorMasterWriteResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<OutsourcedVendorMasterWriteResult>>.Rollback(
            ApplicationResult<OutsourcedVendorMasterWriteResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record OutsourcedVendorState(long RowVersion, DateTimeOffset? DeletedAt);
