...
internal sealed class PostgreSqlFarmerLifecycleExecutor : IFarmerLifecycleExecutor
{
    private const string SoftDeleteCommandType = "SoftDeleteFarmer";
    private const string RestoreCommandType = "RestoreFarmer";
    ...
    public PostgreSqlFarmerLifecycleExecutor(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validationError = FarmerLifecycleValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<FarmerLifecycleResult>.Failure(validationError));
        }
    ...
    private async ValueTask<ApplicationResult<FarmerLifecycleResult>> ExecuteAsync(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<FarmerLifecycleResult>>.Rollback(
                        ApplicationResult<FarmerLifecycleResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        FarmerLifecycleErrorCodes.IdempotencyKeyReused);
                }

                var state = await LockFarmerAsync(farmerId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        FarmerLifecycleErrorCodes.FarmerNotFound);
                }
                ...
                if (state.RowVersion != expectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        FarmerLifecycleErrorCodes.StaleRowVersion);
                }
                ...
                if (operation == LifecycleOperation.Restore && state.DeletedAt is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        FarmerLifecycleErrorCodes.NotDeleted);
                }
                ...
                var result = new FarmerLifecycleResult(
                    farmerId,
                    newRowVersion.Value,
                ...
                return CommandTransactionDecision<ApplicationResult<FarmerLifecycleResult>>.Commit(
                    ApplicationResult<FarmerLifecycleResult>.Success(result));
            },
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Farmer lifecycle CommandExecution conflict could not be loaded.");
        }
        ...
        var replay = JsonSerializer.Deserialize<FarmerLifecycleResult>(
            reader.GetString(4),
            StoredJsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Farmer lifecycle CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Farmer lifecycle SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<FarmerLifecycleResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<FarmerLifecycleResult>>.Rollback(
            ApplicationResult<FarmerLifecycleResult>.Failure(ApplicationError.Create(kind, code)));
    ...
    private sealed record CommandAcquisition(
        ...
        FarmerLifecycleResult? ReplayResult)
    {
        public static CommandAcquisition Acquired() => new(CommandAcquisitionKind.Acquired, null);
        public static CommandAcquisition Replay(FarmerLifecycleResult result) => new(CommandAcquisitionKind.Replay, result);
