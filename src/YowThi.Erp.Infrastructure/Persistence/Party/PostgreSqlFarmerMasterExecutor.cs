...
internal sealed class PostgreSqlFarmerMasterExecutor : IFarmerMasterExecutor
{
    private const string CreateCommandType = "CreateFarmer";
    private const string UpdateCommandType = "UpdateFarmer";
    ...
    public PostgreSqlFarmerMasterExecutor(
    ...
    public ValueTask<ApplicationResult<FarmerMasterWriteResult>> CreateAsync(
        CreateFarmerExecution execution,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(execution);
        var validationError = FarmerMasterValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<FarmerMasterWriteResult>.Failure(validationError));
        }
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var acquisition = await AcquireCommandAsync<FarmerMasterWriteResult>(
                    execution.CommandId,
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<FarmerMasterWriteResult>>.Rollback(
                        ApplicationResult<FarmerMasterWriteResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        FarmerMasterErrorCodes.IdempotencyKeyReused);
                }

                var farmerId = Guid.CreateVersion7();
                await InsertFarmerAsync(
                    farmerId,
                    execution.Command,
                ...
                var result = new FarmerMasterWriteResult(farmerId, 1);
                ...
                return CommandTransactionDecision<ApplicationResult<FarmerMasterWriteResult>>.Commit(
                    ApplicationResult<FarmerMasterWriteResult>.Success(result));
            },
            cancellationToken);
    }

    public ValueTask<ApplicationResult<FarmerMasterWriteResult>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var state = await LockFarmerAsync(execution.Command.FarmerId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        FarmerMasterErrorCodes.FarmerNotFound);
                }
                ...
                if (state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        FarmerMasterErrorCodes.FarmerDeleted);
                }
                ...
                if (state.RowVersion != execution.Command.ExpectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        FarmerMasterErrorCodes.StaleRowVersion);
                }

                var nextRowVersion = await UpdateFarmerAsync(execution.Command, ct);
                ...
                var result = new FarmerMasterWriteResult(
                    execution.Command.FarmerId,
    ...
    private async ValueTask InsertFarmerAsync(
        Guid farmerId,
        ...
        CancellationToken cancellationToken)
    {
        await using var insert = CreateSqlCommand(
            ...
            """);
        insert.Parameters.AddWithValue("id", farmerId);
    ...
    private async ValueTask<FarmerState?> LockFarmerAsync(
        Guid farmerId,
        CancellationToken cancellationToken)
    {
        ...
        return new FarmerState(
            reader.GetInt64(0),
            reader.IsDBNull(1) ? null : reader.GetFieldValue<DateTimeOffset>(1));
    }

    private async ValueTask<long?> UpdateFarmerAsync(
        UpdateFarmerCommand command,
        CancellationToken cancellationToken)
    {
        await using var update = CreateSqlCommand(
            """
            UPDATE party.farmers
            SET code = @code,
            ...
            """);
        update.Parameters.AddWithValue("farmer_id", command.FarmerId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Farmer master CommandExecution conflict could not be loaded.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var subjectKeyJson = JsonSerializer.Serialize(new { id = farmerId }, StoredJsonOptions);
        ...
        await using var subject = CreateSqlCommand(
            ...
            VALUES
                (@audit_event_id, 1, 'party.farmer', CAST(@subject_key AS jsonb), @change_kind,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Farmer master CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Farmer master SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<FarmerMasterWriteResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<FarmerMasterWriteResult>>.Rollback(
            ApplicationResult<FarmerMasterWriteResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record FarmerState(long RowVersion, DateTimeOffset? DeletedAt);
