...
internal sealed class PostgreSqlEmployeeMasterExecutor : IEmployeeMasterExecutor
{
    private const string CreateCommandType = "CreateEmployee";
    private const string UpdateCommandType = "UpdateEmployee";
    ...
    public PostgreSqlEmployeeMasterExecutor(
    ...
    public ValueTask<ApplicationResult<EmployeeMasterWriteResult>> CreateAsync(
        CreateEmployeeExecution execution,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(execution);
        var validationError = EmployeeMasterValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<EmployeeMasterWriteResult>.Failure(validationError));
        }
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var acquisition = await AcquireCommandAsync<EmployeeMasterWriteResult>(
                    execution.CommandId,
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<EmployeeMasterWriteResult>>.Rollback(
                        ApplicationResult<EmployeeMasterWriteResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        EmployeeMasterErrorCodes.IdempotencyKeyReused);
                }
                ...
                var result = new EmployeeMasterWriteResult(employeeId, 1);
                ...
                return CommandTransactionDecision<ApplicationResult<EmployeeMasterWriteResult>>.Commit(
                    ApplicationResult<EmployeeMasterWriteResult>.Success(result));
            },
            cancellationToken);
    }

    public ValueTask<ApplicationResult<EmployeeMasterWriteResult>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var state = await LockEmployeeAsync(execution.Command.EmployeeId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        EmployeeMasterErrorCodes.EmployeeNotFound);
                }
                ...
                if (state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        EmployeeMasterErrorCodes.EmployeeDeleted);
                }
                ...
                if (state.RowVersion != execution.Command.ExpectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        EmployeeMasterErrorCodes.StaleRowVersion);
                }

                var nextRowVersion = await UpdateEmployeeAsync(execution.Command, ct);
                ...
                var result = new EmployeeMasterWriteResult(
    ...
    private async ValueTask InsertEmployeeAsync(
        Guid employeeId,
    ...
    private async ValueTask<long?> UpdateEmployeeAsync(
        ...
        CancellationToken cancellationToken)
    {
        await using var update = CreateSqlCommand(
            ...
            """);
        update.Parameters.AddWithValue("employee_id", command.EmployeeId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Employee master CommandExecution conflict could not be loaded.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Employee master CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Employee master SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<EmployeeMasterWriteResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<EmployeeMasterWriteResult>>.Rollback(
            ApplicationResult<EmployeeMasterWriteResult>.Failure(ApplicationError.Create(kind, code)));
