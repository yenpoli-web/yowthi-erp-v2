...
internal sealed class PostgreSqlCustomerMasterExecutor : ICustomerMasterExecutor
{
    private const string CreateCommandType = "CreateCustomer";
    private const string UpdateCommandType = "UpdateCustomer";
    ...
    public PostgreSqlCustomerMasterExecutor(
    ...
    public ValueTask<ApplicationResult<CustomerMasterWriteResult>> CreateAsync(
        CreateCustomerExecution execution,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(execution);
        var validationError = CustomerMasterValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<CustomerMasterWriteResult>.Failure(validationError));
        }
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var acquisition = await AcquireCommandAsync<CustomerMasterWriteResult>(
                    execution.CommandId,
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<CustomerMasterWriteResult>>.Rollback(
                        ApplicationResult<CustomerMasterWriteResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        CustomerMasterErrorCodes.IdempotencyKeyReused);
                }
                ...
                var result = new CustomerMasterWriteResult(customerId, 1);
                ...
                return CommandTransactionDecision<ApplicationResult<CustomerMasterWriteResult>>.Commit(
                    ApplicationResult<CustomerMasterWriteResult>.Success(result));
            },
            cancellationToken);
    }

    public ValueTask<ApplicationResult<CustomerMasterWriteResult>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var state = await LockCustomerAsync(execution.Command.CustomerId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        CustomerMasterErrorCodes.CustomerNotFound);
                }
                ...
                if (state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        CustomerMasterErrorCodes.CustomerDeleted);
                }
                ...
                if (state.RowVersion != execution.Command.ExpectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        CustomerMasterErrorCodes.StaleRowVersion);
                }

                var nextRowVersion = await UpdateCustomerAsync(execution.Command, ct);
                ...
                var result = new CustomerMasterWriteResult(
                    execution.Command.CustomerId,
    ...
    private async ValueTask InsertCustomerAsync(
        Guid customerId,
    ...
    private async ValueTask<CustomerState?> LockCustomerAsync(
        Guid customerId,
        CancellationToken cancellationToken)
    {
    ...
    private async ValueTask<long?> UpdateCustomerAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Customer master CommandExecution conflict could not be loaded.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var subjectKeyJson = JsonSerializer.Serialize(new { id = customerId }, StoredJsonOptions);
        ...
        await using var subject = CreateSqlCommand(
            ...
            VALUES
                (@audit_event_id, 1, 'party.customer', CAST(@subject_key AS jsonb), @change_kind,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Customer master CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Customer master SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<CustomerMasterWriteResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<CustomerMasterWriteResult>>.Rollback(
            ApplicationResult<CustomerMasterWriteResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record CustomerState(long RowVersion, DateTimeOffset? DeletedAt);
