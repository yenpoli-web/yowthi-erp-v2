...
internal sealed class PostgreSqlSalesPackagingItemMasterExecutor : ISalesPackagingItemMasterExecutor
{
    private const string CreateCommandType = "CreateSalesPackagingItem";
    private const string UpdateCommandType = "UpdateSalesPackagingItem";
    ...
    public PostgreSqlSalesPackagingItemMasterExecutor(
    ...
    public ValueTask<ApplicationResult<SalesPackagingItemMasterWriteResult>> CreateAsync(
        CreateSalesPackagingItemExecution execution,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(execution);
        var validationError = SalesPackagingItemMasterValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<SalesPackagingItemMasterWriteResult>.Failure(validationError));
        }
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var acquisition = await AcquireCommandAsync<SalesPackagingItemMasterWriteResult>(
                    execution.CommandId,
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<SalesPackagingItemMasterWriteResult>>.Rollback(
                        ApplicationResult<SalesPackagingItemMasterWriteResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        SalesPackagingItemMasterErrorCodes.IdempotencyKeyReused);
                }

                var salesPackagingItemId = Guid.CreateVersion7();
                await InsertSalesPackagingItemAsync(
                ...
                var result = new SalesPackagingItemMasterWriteResult(salesPackagingItemId, 1);
                ...
                return CommandTransactionDecision<ApplicationResult<SalesPackagingItemMasterWriteResult>>.Commit(
                    ApplicationResult<SalesPackagingItemMasterWriteResult>.Success(result));
            },
            cancellationToken);
    }

    public ValueTask<ApplicationResult<SalesPackagingItemMasterWriteResult>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                var state = await LockSalesPackagingItemAsync(execution.Command.SalesPackagingItemId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        SalesPackagingItemMasterErrorCodes.ItemNotFound);
                }
                ...
                if (state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesPackagingItemMasterErrorCodes.ItemDeleted);
                }
                ...
                if (state.RowVersion != execution.Command.ExpectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesPackagingItemMasterErrorCodes.StaleRowVersion);
                }

                var nextRowVersion = await UpdateSalesPackagingItemAsync(execution.Command, ct);
                ...
                var result = new SalesPackagingItemMasterWriteResult(
                    execution.Command.SalesPackagingItemId,
    ...
    private async ValueTask InsertSalesPackagingItemAsync(
        ...
        CancellationToken cancellationToken)
    {
        await using var insert = CreateSqlCommand(
            ...
            """);
        insert.Parameters.AddWithValue("id", salesPackagingItemId);
    ...
    private async ValueTask<SalesPackagingItemState?> LockSalesPackagingItemAsync(
        Guid salesPackagingItemId,
        CancellationToken cancellationToken)
    {
        await using var command = CreateSqlCommand(
            ...
            WHERE id = @sales_packaging_item_id
            FOR UPDATE;
            """);
        command.Parameters.AddWithValue("sales_packaging_item_id", salesPackagingItemId);
        ...
        return new SalesPackagingItemState(
            reader.GetInt64(0),
            reader.IsDBNull(1) ? null : reader.GetFieldValue<DateTimeOffset>(1));
    }

    private async ValueTask<long?> UpdateSalesPackagingItemAsync(
        UpdateSalesPackagingItemCommand command,
        CancellationToken cancellationToken)
    {
        await using var update = CreateSqlCommand(
            """
            UPDATE sales_handling.sales_packaging_items
            ...
            """);
        update.Parameters.AddWithValue("sales_packaging_item_id", command.SalesPackagingItemId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Sales Packaging Item master CommandExecution conflict could not be loaded.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var subjectKeyJson = JsonSerializer.Serialize(new { id = salesPackagingItemId }, StoredJsonOptions);
        ...
        await using var subject = CreateSqlCommand(
            ...
            VALUES
                (@audit_event_id, 1, 'sales-handling.packaging-item', CAST(@subject_key AS jsonb), @change_kind,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Sales Packaging Item master CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Sales Packaging Item master SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<SalesPackagingItemMasterWriteResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<SalesPackagingItemMasterWriteResult>>.Rollback(
            ApplicationResult<SalesPackagingItemMasterWriteResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record SalesPackagingItemState(long RowVersion, DateTimeOffset? DeletedAt);
