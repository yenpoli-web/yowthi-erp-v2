...
internal sealed class PostgreSqlSalesPackagingItemLifecycleExecutor : ISalesPackagingItemLifecycleExecutor
{
    private const string SoftDeleteCommandType = "SoftDeleteSalesPackagingItem";
    private const string RestoreCommandType = "RestoreSalesPackagingItem";
    ...
    public PostgreSqlSalesPackagingItemLifecycleExecutor(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validationError = SalesPackagingItemLifecycleValidation.Validate(execution.Command);
        if (validationError is not null)
        {
            return ValueTask.FromResult(ApplicationResult<SalesPackagingItemLifecycleResult>.Failure(validationError));
        }
    ...
    private async ValueTask<ApplicationResult<SalesPackagingItemLifecycleResult>> ExecuteAsync(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<SalesPackagingItemLifecycleResult>>.Rollback(
                        ApplicationResult<SalesPackagingItemLifecycleResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        SalesPackagingItemLifecycleErrorCodes.IdempotencyKeyReused);
                }

                var state = await LockItemAsync(salesPackagingItemId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        SalesPackagingItemLifecycleErrorCodes.ItemNotFound);
                }
                ...
                if (state.RowVersion != expectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesPackagingItemLifecycleErrorCodes.StaleRowVersion);
                }
                ...
                if (operation == LifecycleOperation.SoftDelete && state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesPackagingItemLifecycleErrorCodes.AlreadyDeleted);
                }
                ...
                if (operation == LifecycleOperation.Restore && state.DeletedAt is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        SalesPackagingItemLifecycleErrorCodes.NotDeleted);
                }
                ...
                var result = new SalesPackagingItemLifecycleResult(
                    salesPackagingItemId,
                    newRowVersion.Value,
                ...
                return CommandTransactionDecision<ApplicationResult<SalesPackagingItemLifecycleResult>>.Commit(
                    ApplicationResult<SalesPackagingItemLifecycleResult>.Success(result));
            },
            cancellationToken);
    }

    private async ValueTask<ItemState?> LockItemAsync(Guid salesPackagingItemId, CancellationToken cancellationToken)
    {
        await using var command = CreateSqlCommand(
            ...
            FOR UPDATE;
            """);
        command.Parameters.AddWithValue("item_id", salesPackagingItemId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
        {
            throw new InvalidOperationException("Sales Packaging Item lifecycle CommandExecution conflict could not be loaded.");
        }
        ...
        var replay = JsonSerializer.Deserialize<SalesPackagingItemLifecycleResult>(reader.GetString(4), StoredJsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Sales Packaging Item lifecycle CommandExecution could not transition to SUCCEEDED.");
        }
    }
    ...
    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Sales Packaging Item lifecycle SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<SalesPackagingItemLifecycleResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<SalesPackagingItemLifecycleResult>>.Rollback(
            ApplicationResult<SalesPackagingItemLifecycleResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record ItemState(long RowVersion, DateTimeOffset? DeletedAt);
    ...
    private sealed record CommandAcquisition(
        ...
        SalesPackagingItemLifecycleResult? ReplayResult)
    {
        public static CommandAcquisition Acquired() => new(CommandAcquisitionKind.Acquired, null);
        public static CommandAcquisition Replay(SalesPackagingItemLifecycleResult result) => new(CommandAcquisitionKind.Replay, result);
