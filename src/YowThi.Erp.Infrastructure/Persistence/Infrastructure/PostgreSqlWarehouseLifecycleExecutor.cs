...
internal sealed class PostgreSqlWarehouseLifecycleExecutor : IWarehouseLifecycleExecutor
{
    private const string SoftDeleteCommandType = "SoftDeleteWarehouse";
    private const string RestoreCommandType = "RestoreWarehouse";
    ...
    public PostgreSqlWarehouseLifecycleExecutor(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validationError = WarehouseLifecycleValidation.Validate(execution.Command);
        if (validationError is not null)
            return ValueTask.FromResult(ApplicationResult<WarehouseLifecycleResult>.Failure(validationError));

        return ExecuteAsync(
            execution.CommandId,
            execution.RequestHash,
            execution.ActorAccountId,
            execution.Command.WarehouseId,
    ...
    private async ValueTask<ApplicationResult<WarehouseLifecycleResult>> ExecuteAsync(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<WarehouseLifecycleResult>>.Rollback(
                        ApplicationResult<WarehouseLifecycleResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        WarehouseLifecycleErrorCodes.IdempotencyKeyReused);
                }

                var state = await LockWarehouseAsync(warehouseId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        WarehouseLifecycleErrorCodes.WarehouseNotFound);
                }
                ...
                if (state.RowVersion != expectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        WarehouseLifecycleErrorCodes.StaleRowVersion);
                }
                ...
                if (operation == LifecycleOperation.SoftDelete && state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        WarehouseLifecycleErrorCodes.AlreadyDeleted);
                }
                ...
                if (operation == LifecycleOperation.Restore && state.DeletedAt is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        WarehouseLifecycleErrorCodes.NotDeleted);
                }
                ...
                var result = new WarehouseLifecycleResult(
                    warehouseId,
                    newRowVersion.Value,
                ...
                return CommandTransactionDecision<ApplicationResult<WarehouseLifecycleResult>>.Commit(
                    ApplicationResult<WarehouseLifecycleResult>.Success(result));
            },
            cancellationToken);
    }

    private async ValueTask<WarehouseState?> LockWarehouseAsync(Guid warehouseId, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
            throw new InvalidOperationException("Warehouse lifecycle CommandExecution conflict could not be loaded.");
        ...
        var replay = JsonSerializer.Deserialize<WarehouseLifecycleResult>(reader.GetString(4), StoredJsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var subjectKeyJson = JsonSerializer.Serialize(new { id = warehouseId }, StoredJsonOptions);
        ...
        await using var subject = CreateSqlCommand(
            ...
            VALUES
                (@audit_event_id, 1, 'infrastructure.warehouse', CAST(@subject_key AS jsonb), @change_kind,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await command.ExecuteNonQueryAsync(cancellationToken) != 1)
            throw new InvalidOperationException("Warehouse lifecycle CommandExecution could not transition to SUCCEEDED.");
    }

    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Warehouse lifecycle SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<WarehouseLifecycleResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<WarehouseLifecycleResult>>.Rollback(
            ApplicationResult<WarehouseLifecycleResult>.Failure(ApplicationError.Create(kind, code)));
    ...
    private sealed record CommandAcquisition(
        ...
        WarehouseLifecycleResult? ReplayResult)
    {
        public static CommandAcquisition Acquired() => new(CommandAcquisitionKind.Acquired, null);
        public static CommandAcquisition Replay(WarehouseLifecycleResult result) => new(CommandAcquisitionKind.Replay, result);
