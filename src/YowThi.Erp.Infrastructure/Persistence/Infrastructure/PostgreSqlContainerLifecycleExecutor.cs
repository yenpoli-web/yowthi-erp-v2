...
internal sealed class PostgreSqlContainerLifecycleExecutor : IContainerLifecycleExecutor
{
    private const string SoftDeleteCommandType = "SoftDeleteContainer";
    private const string RestoreCommandType = "RestoreContainer";
    ...
    public PostgreSqlContainerLifecycleExecutor(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var validationError = ContainerLifecycleValidation.Validate(execution.Command);
        if (validationError is not null)
            return ValueTask.FromResult(ApplicationResult<ContainerLifecycleResult>.Failure(validationError));

        return ExecuteAsync(
            execution.CommandId,
            execution.RequestHash,
            execution.ActorAccountId,
            execution.Command.ContainerId,
    ...
    private async ValueTask<ApplicationResult<ContainerLifecycleResult>> ExecuteAsync(
        CommandId commandId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _transactionRunner.ExecuteAsync(
            async ct =>
            {
                ...
                {
                    return CommandTransactionDecision<ApplicationResult<ContainerLifecycleResult>>.Rollback(
                        ApplicationResult<ContainerLifecycleResult>.Success(acquisition.ReplayResult!));
                }
                ...
                if (acquisition.Kind == CommandAcquisitionKind.Conflict)
                {
                    return RollbackFailure(
                        ...
                        ContainerLifecycleErrorCodes.IdempotencyKeyReused);
                }

                var state = await LockContainerAsync(containerId, ct);
                if (state is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.NotFound,
                        ContainerLifecycleErrorCodes.ContainerNotFound);
                }
                ...
                if (state.RowVersion != expectedRowVersion)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        ContainerLifecycleErrorCodes.StaleRowVersion);
                }
                ...
                if (operation == LifecycleOperation.SoftDelete && state.DeletedAt is not null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        ContainerLifecycleErrorCodes.AlreadyDeleted);
                }
                ...
                if (operation == LifecycleOperation.Restore && state.DeletedAt is null)
                {
                    return RollbackFailure(
                        ApplicationErrorKind.Conflict,
                        ContainerLifecycleErrorCodes.NotDeleted);
                }
                ...
                var result = new ContainerLifecycleResult(
                    containerId,
                    newRowVersion.Value,
                ...
                return CommandTransactionDecision<ApplicationResult<ContainerLifecycleResult>>.Commit(
                    ApplicationResult<ContainerLifecycleResult>.Success(result));
            },
            cancellationToken);
    }

    private async ValueTask<ContainerState?> LockContainerAsync(Guid containerId, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await reader.ReadAsync(cancellationToken))
            throw new InvalidOperationException("Container lifecycle CommandExecution conflict could not be loaded.");
        ...
        var replay = JsonSerializer.Deserialize<ContainerLifecycleResult>(reader.GetString(4), StoredJsonOptions)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var subjectKeyJson = JsonSerializer.Serialize(new { id = containerId }, StoredJsonOptions);
        ...
        await using var subject = CreateSqlCommand(
            ...
            VALUES
                (@audit_event_id, 1, 'infrastructure.container', CAST(@subject_key AS jsonb), @change_kind,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await command.ExecuteNonQueryAsync(cancellationToken) != 1)
            throw new InvalidOperationException("Container lifecycle CommandExecution could not transition to SUCCEEDED.");
    }

    private NpgsqlCommand CreateSqlCommand(string sql)
    {
        var transaction = _dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Container lifecycle SQL requires an active command transaction.");
    ...
    private static CommandTransactionDecision<ApplicationResult<ContainerLifecycleResult>> RollbackFailure(
        ApplicationErrorKind kind,
        string code) =>
        CommandTransactionDecision<ApplicationResult<ContainerLifecycleResult>>.Rollback(
            ApplicationResult<ContainerLifecycleResult>.Failure(ApplicationError.Create(kind, code)));

    private sealed record ContainerState(long RowVersion, DateTimeOffset? DeletedAt);
    ...
    private sealed record CommandAcquisition(
        ...
        ContainerLifecycleResult? ReplayResult)
    {
        public static CommandAcquisition Acquired() => new(CommandAcquisitionKind.Acquired, null);
        public static CommandAcquisition Replay(ContainerLifecycleResult result) => new(CommandAcquisitionKind.Replay, result);
