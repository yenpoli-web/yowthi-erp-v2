...
public sealed class SupplierMasterIntegrationTests
{
    ...
    [Fact]
    public async Task Create_and_update_supplier_persist_full_master_data_with_audit_and_idempotent_replay()
    {
        ...
        try
        {
            var createExecution = new CreateSupplierExecution(
                CommandId.From(createCommandId), Hash(1), ActorAccountId.From(actorId),
                new CreateSupplierCommand("供應商整合測試", "ผู้จำหน่ายทดสอบ", "KBANK", "123-4", "0812345678", "Chiang Mai", true));
            var created = await ExecuteCreateAsync(createExecution, ct);
            Assert.True(created.IsSuccess);
            supplierId = created.Value.SupplierId;
            Assert.NotEqual(Guid.Empty, supplierId);
            Assert.Equal(1, created.Value.RowVersion);

            var row = await ReadSupplierAsync(supplierId, ct);
            Assert.Equal("供應商整合測試", row.NameZhTw);
            Assert.Equal("ผู้จำหน่ายทดสอบ", row.NameThTh);
            ...
            Assert.Equal(1L, await AuditCountAsync(createCommandId, "CreateSupplier", "CREATE", null, 1, ct));
            var replay = await ExecuteCreateAsync(createExecution, ct);
            Assert.True(replay.IsSuccess);
            Assert.Equal(created.Value, replay.Value);
            Assert.Equal(1L, await ScalarAsync<long>("SELECT count(*) FROM party.suppliers WHERE id = @id;", ct, ("id", supplierId)));
            Assert.Equal(1L, await ScalarAsync<long>("SELECT count(*) FROM audit.audit_events WHERE command_id = @id;", ct, ("id", createCommandId)));

            var updateExecution = new UpdateSupplierExecution(
                CommandId.From(updateCommandId), Hash(2), ActorAccountId.From(actorId),
                new UpdateSupplierCommand(supplierId, 1, "供應商更新", null, null, null, "0899999999", "Bangkok", false));
            var updated = await ExecuteUpdateAsync(updateExecution, ct);
            Assert.True(updated.IsSuccess);
            Assert.Equal(new SupplierMasterWriteResult(supplierId, 2), updated.Value);

            row = await ReadSupplierAsync(supplierId, ct);
            Assert.Equal("供應商更新", row.NameZhTw);
            ...
            Assert.Equal(1L, await AuditCountAsync(updateCommandId, "UpdateSupplier", "UPDATE", 1, 2, ct));

            var page = await ReadMasterAsync("供應商更新", SupplierMasterStatusFilter.Inactive, ct);
            var projected = Assert.Single(page.Items, item => item.Id == supplierId);
            Assert.Equal("0899999999", projected.Phone);
            Assert.Equal(2, projected.RowVersion);
        }
        finally
        {
            await CleanupAsync(actorId, supplierId, [createCommandId, updateCommandId], ct);
        }
    }

    [Fact]
    public async Task Update_stale_or_deleted_supplier_rolls_back_command_acquisition()
    {
        var ct = TestContext.Current.CancellationToken;
        var actorId = Guid.CreateVersion7();
        ...
        await ExecuteNonQueryAsync(
            ...
            VALUES (@id, '供應商', NULL, NULL, NULL, NULL, NULL, true, 1, now(), @actor);
            """, ct, ("id", supplierId), ("actor", actorId));

        try
        {
            var stale = await ExecuteUpdateAsync(new UpdateSupplierExecution(
                CommandId.From(staleId), Hash(3), ActorAccountId.From(actorId),
                new UpdateSupplierCommand(supplierId, 2, "供應商", null, null, null, null, null, true)), ct);
            Assert.True(stale.IsFailure);
            Assert.Equal(ApplicationErrorKind.Conflict, stale.Error.Kind);
            Assert.Equal(SupplierMasterErrorCodes.StaleRowVersion, stale.Error.Code);

            await ExecuteNonQueryAsync(
                "UPDATE party.suppliers SET deleted_at = now(), deleted_by_account_id = @actor, row_version = 2 WHERE id = @id;",
                ct, ("actor", actorId), ("id", supplierId));
            var deleted = await ExecuteUpdateAsync(new UpdateSupplierExecution(
                CommandId.From(deletedId), Hash(4), ActorAccountId.From(actorId),
                new UpdateSupplierCommand(supplierId, 2, "供應商", null, null, null, null, null, true)), ct);
            Assert.True(deleted.IsFailure);
            Assert.Equal(SupplierMasterErrorCodes.SupplierDeleted, deleted.Error.Code);
        ...
        {
            await CleanupAsync(actorId, supplierId, [staleId, deletedId], ct);
        }
    }

    private static async ValueTask<ApplicationResult<SupplierMasterWriteResult>> ExecuteCreateAsync(CreateSupplierExecution execution, CancellationToken ct)
    {
        var services = new ServiceCollection();
        services.AddErpPersistence(GetConnectionString());
        await using var provider = services.BuildServiceProvider();
        await using var scope = provider.CreateAsyncScope();
        return await scope.ServiceProvider.GetRequiredService<ISupplierMasterExecutor>().CreateAsync(execution, ct);
    }

    private static async ValueTask<ApplicationResult<SupplierMasterWriteResult>> ExecuteUpdateAsync(UpdateSupplierExecution execution, CancellationToken ct)
    {
        var services = new ServiceCollection();
        services.AddErpPersistence(GetConnectionString());
        await using var provider = services.BuildServiceProvider();
        await using var scope = provider.CreateAsyncScope();
        return await scope.ServiceProvider.GetRequiredService<ISupplierMasterExecutor>().UpdateAsync(execution, ct);
    }

    private static async ValueTask<SupplierMasterPage> ReadMasterAsync(string search, SupplierMasterStatusFilter status, CancellationToken ct)
    {
        var services = new ServiceCollection();
        services.AddErpPersistence(GetConnectionString());
        await using var provider = services.BuildServiceProvider();
        await using var scope = provider.CreateAsyncScope();
        return await scope.ServiceProvider.GetRequiredService<ISupplierMasterReader>().GetAsync(new(search, status, 0, 100), ct);
    }
    ...
    private static Task SeedActorAsync(Guid actorId, CancellationToken ct) => ExecuteNonQueryAsync(
        "INSERT INTO system.accounts (id, display_name, active, identity_issuer, identity_subject, created_at) VALUES (@id, 'P8 supplier master actor', true, NULL, NULL, now());",
        ct, ("id", actorId));

    private static async Task<SupplierDbRow> ReadSupplierAsync(Guid id, CancellationToken ct)
    {
        await using var connection = await OpenConnectionAsync(ct);
        await using var command = new NpgsqlCommand(
            "SELECT name_zh_tw, name_th_th, bank_name, bank_account, phone, address, active, row_version, created_by_account_id, deleted_at FROM party.suppliers WHERE id = @id;",
            connection);
    ...
    private static Task<long> AuditCountAsync(Guid commandId, string commandType, string changeKind, long? before, long after, CancellationToken ct) => ScalarAsync<long>(
        ...
        WHERE e.command_id = @command_id AND e.command_type = @command_type
          AND e.event_kind = 'BUSINESS_COMMAND' AND s.subject_kind = 'party.supplier'
    ...
    private static async Task CleanupAsync(Guid actorId, Guid supplierId, IReadOnlyCollection<Guid> commandIds, CancellationToken ct)
    {
        ...
        if (supplierId != Guid.Empty) await ExecuteNonQueryAsync("DELETE FROM party.suppliers WHERE id = @id;", ct, ("id", supplierId));
    ...
    private sealed record SupplierDbRow(string? NameZhTw, string? NameThTh, string? BankName, string? BankAccount, string? Phone, string? Address, bool Active, long RowVersion, Guid CreatedByAccountId, DateTimeOffset? DeletedAt);
}
