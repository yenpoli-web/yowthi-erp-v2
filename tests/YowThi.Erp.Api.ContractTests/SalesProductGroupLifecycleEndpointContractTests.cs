...
public sealed class SalesProductGroupLifecycleEndpointContractTests
{
    private const string SoftDeleteRoute = "/api/v1/product/sales-product-groups/{salesProductGroupId:guid}/soft-delete";
    private const string RestoreRoute = "/api/v1/product/sales-product-groups/{salesProductGroupId:guid}/restore";

    [Fact]
    public async Task Sales_product_group_lifecycle_routes_are_target_specific_authorized_idempotent_v1_posts()
    {
        var fixture = CreateApp();
        await using var app = fixture.App;
        app.MapSalesProductGroupLifecycleEndpoints();

        AssertLifecycleEndpoint(GetEndpoint(app, SoftDeleteRoute), SalesProductGroupLifecycleEndpoints.SoftDeleteOperationId);
        AssertLifecycleEndpoint(GetEndpoint(app, RestoreRoute), SalesProductGroupLifecycleEndpoints.RestoreOperationId);
    }

    [Fact]
    public async Task Sales_product_group_lifecycle_canonical_commands_include_route_id_and_expected_row_version()
    {
        var fixture = CreateApp();
        await using var app = fixture.App;
        app.MapSalesProductGroupLifecycleEndpoints();
        ...
        fixture.Executor.SoftDeleteResult = ApplicationResult<SalesProductGroupLifecycleResult>.Success(
            new SalesProductGroupLifecycleResult(groupId, 8, true));
        ...
        Assert.Equal(groupId, fixture.Executor.LastSoftDeleteExecution!.Command.SalesProductGroupId);
        Assert.Equal(7, fixture.Executor.LastSoftDeleteExecution.Command.ExpectedRowVersion);
        AssertCanonical(fixture.Hasher.LastPayloadJson, SalesProductGroupLifecycleEndpoints.SoftDeleteCommandType, groupId, 7);

        fixture.Executor.RestoreResult = ApplicationResult<SalesProductGroupLifecycleResult>.Success(
            new SalesProductGroupLifecycleResult(groupId, 9, false));
        ...
        Assert.Equal(groupId, fixture.Executor.LastRestoreExecution!.Command.SalesProductGroupId);
        Assert.Equal(8, fixture.Executor.LastRestoreExecution.Command.ExpectedRowVersion);
        AssertCanonical(fixture.Hasher.LastPayloadJson, SalesProductGroupLifecycleEndpoints.RestoreCommandType, groupId, 8);
    }

    [Fact]
    public async Task Sales_product_group_lifecycle_maps_conflicts_and_does_not_reuse_business_fact_or_hard_delete_policies()
    {
        var fixture = CreateApp();
        await using var app = fixture.App;
        app.MapSalesProductGroupLifecycleEndpoints();

        fixture.Executor.SoftDeleteResult = ApplicationResult<SalesProductGroupLifecycleResult>.Failure(
            ApplicationError.Create(ApplicationErrorKind.Conflict, SalesProductGroupLifecycleErrorCodes.AlreadyDeleted));
        ...
        AssertProblemCode(conflict.Body, SalesProductGroupLifecycleErrorCodes.AlreadyDeleted);
        ...
        var policies = GetEndpoint(app, SoftDeleteRoute).Metadata.GetOrderedMetadata<IAuthorizeData>()
            .Select(x => x.Policy)
            .ToArray();
        Assert.Contains(CapabilityPolicies.SalesProductGroupLifecycle, policies);
        Assert.DoesNotContain(CapabilityPolicies.SalesConfirm, policies);
        Assert.DoesNotContain(CapabilityPolicies.SalesPackagingItemLifecycle, policies);
    ...
    private static void AssertLifecycleEndpoint(RouteEndpoint endpoint, string operationId)
    {
        ...
        Assert.Contains(
            ...
            metadata => metadata.Policy == CapabilityPolicies.SalesProductGroupLifecycle);
        Assert.NotNull(endpoint.Metadata.GetMetadata<RequiresIdempotencyKeyMetadata>());
    }

    private static void AssertCanonical(string? payload, string commandType, Guid groupId, long expectedRowVersion)
    {
        ...
        Assert.Equal(groupId, command.GetProperty("salesProductGroupId").GetGuid());
        ...
        string body)
    {
        ...
        context.Request.Path = endpoint.RoutePattern.RawText!
            .Replace("{salesProductGroupId:guid}", groupId.ToString(), StringComparison.Ordinal);
        context.Request.RouteValues["salesProductGroupId"] = groupId.ToString();
    ...
    private static AppFixture CreateApp()
    {
        ...
        builder.Services.AddSingleton<ISalesProductGroupLifecycleExecutor>(executor);
    ...
    private sealed class StubExecutor : ISalesProductGroupLifecycleExecutor
    {
        public ApplicationResult<SalesProductGroupLifecycleResult> SoftDeleteResult { get; set; } =
            ApplicationResult<SalesProductGroupLifecycleResult>.Success(
                new SalesProductGroupLifecycleResult(Guid.NewGuid(), 2, true));
        public ApplicationResult<SalesProductGroupLifecycleResult> RestoreResult { get; set; } =
            ApplicationResult<SalesProductGroupLifecycleResult>.Success(
                new SalesProductGroupLifecycleResult(Guid.NewGuid(), 3, false));
        public SoftDeleteSalesProductGroupExecution? LastSoftDeleteExecution { get; private set; }
        public RestoreSalesProductGroupExecution? LastRestoreExecution { get; private set; }

        public ValueTask<ApplicationResult<SalesProductGroupLifecycleResult>> SoftDeleteAsync(
        ...
        public ValueTask<ApplicationResult<SalesProductGroupLifecycleResult>> RestoreAsync(
