...
public sealed class FarmerHardDeleteEndpointContractTests
{
    private const string Route = "/api/v1/data-protection/farmers/{farmerId:guid}/hard-delete";

    [Fact]
    public async Task Farmer_hard_delete_is_target_specific_authorized_idempotent_v1_post()
    {
        ...
        Assert.Equal(
            DataProtectionEndpoints.HardDeleteFarmerOperationId,
    ...
    [Fact]
    public async Task Farmer_hard_delete_canonical_command_includes_route_id_and_expected_row_version()
    {
        var fixture = CreateApp();
        await using var app = fixture.App;
        app.MapDataProtectionEndpoints();
        var farmerId = Guid.NewGuid();
        fixture.Executor.Result = ApplicationResult<HardDeleteFarmerResult>.Success(
            new HardDeleteFarmerResult(farmerId));

        var response = await InvokeAsync(app, GetEndpoint(app), farmerId, Guid.NewGuid(), """{"expectedRowVersion":7}""");
        Assert.Equal(StatusCodes.Status200OK, response.StatusCode);
        Assert.Equal(farmerId, fixture.Executor.LastExecution!.Command.FarmerId);
        ...
        Assert.Equal(
            DataProtectionEndpoints.HardDeleteFarmerCommandType,
    ...
    [Fact]
    public async Task Farmer_hard_delete_maps_dependency_block_and_transport_validation()
    {
        var fixture = CreateApp();
        await using var app = fixture.App;
        app.MapDataProtectionEndpoints();
        ...
        fixture.Executor.Result = ApplicationResult<HardDeleteFarmerResult>.Failure(
        ...
        AssertProblemCode(blocked.Body, FarmerHardDeleteErrorCodes.DependencyBlocked);
    ...
    private static AppFixture CreateApp()
    {
        ...
        builder.Services.AddSingleton<IHardDeleteFarmerExecutor>(executor);
    ...
    private sealed class CapturingRequestHasher : ICommandRequestHasher
    {
        ...
        public CommandRequestHash Compute(JsonPayload canonicalCommandPayload)
        {
            ...
            return CommandRequestHash.FromSha256(
                Enumerable.Repeat((byte)0x6E, CommandRequestHash.Sha256Length).ToArray());
        }
    }

    private sealed class StubExecutor : IHardDeleteFarmerExecutor
    {
        public ApplicationResult<HardDeleteFarmerResult> Result { get; set; } =
            ApplicationResult<HardDeleteFarmerResult>.Success(
                new HardDeleteFarmerResult(Guid.NewGuid()));
        public HardDeleteFarmerExecution? LastExecution { get; private set; }

        public ValueTask<ApplicationResult<HardDeleteFarmerResult>> ExecuteAsync(
